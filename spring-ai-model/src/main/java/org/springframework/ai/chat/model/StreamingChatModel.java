 ...
 *
 * <p>
 * 该接口为支持「逐 token / 逐片段返回」的聊天模型提供统一的抽象，是 {@link ChatModel}
 * 的流式版本。它继承自通用流式模型抽象 {@link StreamingModel}，并基于 Reactor
 * {@link Flux} 暴露非阻塞、背压友好的响应序列。
 *
 * <p>
 * 接口被声明为 {@link FunctionalInterface @FunctionalInterface}，因此可以使用
 * Lambda 表达式或方法引用快速实现一个流式模型，常见于测试 / Mock 场景：
 * <pre>{@code
 * StreamingChatModel mock = prompt -> Flux.just(response1, response2);
 * }</pre>
 *
 * <p>
 * 同样提供了 {@code String} 与 {@code Message...} 两种便捷重载，将每段
 * {@link ChatResponse} 中的 {@link AssistantMessage} 文本平铺为 {@code Flux<String>}，
 * 便于直接绑定到 SSE 端点或终端输出。
 *
 * @see ChatModel
 * @see StreamingModel
...
public interface StreamingChatModel extends StreamingModel<Prompt, ChatResponse> {
	 ...
	 * <p>
	 * 当某一段响应的 {@link Generation} 或 {@link AssistantMessage#getText()} 为
	 * {@code null} 时，会被映射为空字符串而不是抛出 NPE，因此下游可以放心地直接
	 * 拼接结果。
