 ...
 * 它只关心"<i>消息存到哪里、怎么按会话 ID 增删改查</i>"，不关心窗口、摘要等
 * 高层策略。Spring AI 为它提供了多种实现：
 * <ul>
 *   <li>{@link InMemoryChatMemoryRepository}：基于 {@code ConcurrentHashMap} 的进程内实现，开发与测试用；</li>
 ...
 * <h2>注意</h2>
 * <p>
 * {@link #saveAll(String, List)} 是<b>整段替换</b>而非追加——这把"如何合并旧消息与新消息"
 * 的责任明确推给了上层 {@link ChatMemory} 实现，存储层只做幂等的整段写入，
 * 便于在分布式环境下保持一致性。
 *
...
public interface ChatMemoryRepository {
	 ...
	 * messages.
	 * <p>
	 * <b>整段替换语义</b>：用 {@code messages} 完全覆盖该会话已有的消息列表，
	 * 上层负责先把旧消息和新消息合并好再传入。
	 */
