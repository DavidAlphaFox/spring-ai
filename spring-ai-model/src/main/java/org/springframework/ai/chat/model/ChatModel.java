 ...
 *
 * <p>
 * 该接口是 Spring AI 聊天能力的顶层抽象，定义了与底层大语言模型（LLM）进行 一问一答交互的统一编程模型。它继承自：
 * <ul>
 * <li>{@link Model}&lt;{@link Prompt}, {@link ChatResponse}&gt; —— 通用模型抽象， 表示「输入
 * Prompt，输出 ChatResponse」的同步调用语义。</li>
 * <li>{@link StreamingChatModel} —— 流式聊天模型抽象，提供基于 Reactor {@link Flux} 的增量式响应能力。</li>
 * </ul>
 *
 * <p>
 * 各厂商（OpenAI、Anthropic、Azure、Ollama 等）的具体实现类只需实现 {@link #call(Prompt)} 与
 * {@link #stream(Prompt)} 即可对外提供统一的聊天能力， 上层应用（ChatClient、Advisor、Agent
 * 等）可基于该接口编写与厂商无关的代码。
 *
 * <p>
 * 接口同时提供了一组便捷的默认方法（接收 {@code String}、{@code Message...}）， 适用于无需自定义 {@link ChatOptions}
 * 的简单场景，避免每次都手动构造 {@link Prompt}。
 *
 * @author Mark Pollack
...
public interface ChatModel extends Model<Prompt, ChatResponse>, StreamingChatModel {
	 ...
	 * <p>
	 * 内部会将传入的字符串包装为一条 {@link UserMessage}，再封装为 {@link Prompt} 调用 {@link #call(Prompt)}。
	 * @param message 用户输入的文本内容
	 * @return 模型生成的文本结果；当模型未返回任何 {@link Generation} 时返回空字符串， 当 {@code Generation} 为
	 ...
	 * <p>
	 * 适用于需要传入上下文（多轮对话历史 / system prompt 等）但又不希望使用 {@link ChatOptions} 的场景。
	 ...
	 * <p>
	 * 该方法是接口的核心抽象点：上面的便捷重载、ChatClient、Advisor 等最终都会 收敛到此方法。
	 * @param prompt 完整的提示词对象，包含消息列表及调用选项
	 * @return 模型返回的 {@link ChatResponse}，包含一个或多个 {@link Generation} 以及响应级元数据（如 token
	 * 用量、模型名等）
	 */
	@Override
	 ...
	 * <p>
	 * 当调用方没有在 {@link Prompt} 中显式指定选项时，框架将退回使用此默认值。 子类应覆写本方法以提供模型特有的默认参数（如默认
	 * model、temperature 等）。
	 ...
	 * 这样不支持流式的实现类可以直接复用。注意：尽管接口同时继承了 {@link StreamingChatModel}（其 {@code stream}
	 * 是抽象方法），但具体的 {@link ChatModel} 实现类如果不打算支持流式，可以保留此默认实现。
